    ...
    async getCashCuts(limit = 50) {
        if (this.useDatabase) {
            ...
            return result.rows.map(row => ({
                ...
                totals: row.totals,
                expectedCash: row.expected_cash ? parseFloat(row.expected_cash) : 0,
                actualCash: row.actual_cash ? parseFloat(row.actual_cash) : 0,
                difference: row.difference ? parseFloat(row.difference) : 0,
                notes: row.notes,
